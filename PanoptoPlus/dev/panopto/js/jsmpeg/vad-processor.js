...
VADProcessor = (() => {
    ...
    class VADProcessor {
        ...
        constructor(inputs, options) {
            ...
            this.currentTime = 0;
            //48,000 samples per second, 1 interval is every 128
            this.timePerInterval = 128 / this.sampleRate;
            this.results = [];
         ...
         * @returns {Results|undefined} undefined if it was a noise sample
         */
        process() {
            if (this.processedResults) return this.results;
            //trim excess that we don't have enough info to process
            const length = this.float32Length - (this.float32Length % FFT1_BIN_COUNT);
            for (let ofs = 0; ofs < length; ofs += FFT1_BIN_COUNT) {
        ...
        processReference() {
            ...
            if (this.referenceSamples < NOISE_SAMPLE_COUNT) {
