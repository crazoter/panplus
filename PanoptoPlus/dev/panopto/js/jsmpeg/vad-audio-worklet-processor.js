...
class VADAudioWorkletProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.silenceTimeThreshold = 1;
        this.logGraphData = {};
        this.sampleRate = 48000;
        this.startProcessingFrom = 0;
        ...
        this.port.onmessage = (function(event) {
            switch (event.data.msgEnum) {
                case MessageEnums.INITIALIZATION_PARAMS:
                    this.startProcessingFrom = event.data.data.options.startProcessingFrom;
                    if (event.data.data.noiseReferenceLineFeatures != null) {
                        this.msgEnum = MessageEnums.NORMAL_RESULTS;
                        this.referenceLineFeatures = event.data.data.noiseReferenceLineFeatures;
                ...
                case MessageEnums.REQUEST_RESULTS: 
                    ...
                    this.port.postMessage({data: {results: this.results, interval: this.calculateLagTime()}, msgEnum: this.msgEnum});
                    //this.sendGraphData();
                    break;
            }
        }).bind(this);
    ...
    process(inputs, outputs, parameters) {
        ...
        if (currentTime >= this.startProcessingFrom) {
    ...
    processReference(parameters) {
        ...
        if (this.referenceSamples == null) {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                for (let j = 0; j < 5; j++) {
                    this.referenceData[i].push([]);
                    this.results[i].push({mean: 0, stddev: 0});
                }
            }
        }
        if (this.referenceSamples < this.noiseSampleCount) {
            let newData = this.processData(parameters, false);
            this.referenceSamples++;
            for (let i = 0; i < this.referenceData.length; i++) {
                for (let j = 0; j < this.referenceData[i].length; j++) {
                    ////Push to array of data
                    this.referenceData[i][j].push(newData[i][j]);
                    //Sum for mean later
                    this.results[i][j].mean += newData[i][j];
                }
            }
            ...
            if (this.referenceSamples >= this.noiseSampleCount) {
                ...
                for (let i = 0; i < this.referenceData.length; i++) {
                    for (let j = 0; j < this.referenceData[i].length; j++) {
                        //Calculate mean
                        this.results[i][j].mean /= this.referenceData[i][j].length;
                        //Calculate variance & subsequently std dev
                        for (let k = 0; k < this.referenceData[i][j].length; k++) {
                            this.results[i][j].stddev += Math.pow(this.referenceData[i][j][k] - this.results[i][j].mean, 2);
                        }
                        this.results[i][j].stddev /= this.referenceData[i][j].length - 1;
                        this.results[i][j].stddev = Math.sqrt(this.results[i][j].stddev);
                        //Calculate the final bounds for each variable
                        this.results[i][j] = {
                            lowerBound: this.results[i][j].mean - this.results[i][j].stddev * this.stddevDistance[j], 
                            upperBound: this.results[i][j].mean + this.results[i][j].stddev * this.stddevDistance[j]
                        };
                    }
                }
    ...
    processData(parameters, checkForSpeech = true) {
        ...
        for (let i = 0; i < this.buffer.length; i++) {
            ...
            if (checkForSpeech) {
                this.recordDistance(this.calculateDistanceMeasurement(lineFeatures[i], this.referenceLineFeatures[i]));
            }*/
            if (checkForSpeech && this.distanceOutsideBounds(lineFeatures[i], this.referenceLineFeatures[i])) {
                isSpeaking = true;
                break;
            }
        }
    ...
    calculateLineFeatures(magArr, magArrLength) {
        ...
        for (let i = 0; i < lineFeatures.length; i++) {
            if (!isFinite(lineFeatures[i])) {
                this.log("NaN LF Before", {magArr: magArr, lineFeatures: lineFeatures, sumMag: sumMag, sumMagDivj: sumMagDivj, sumMagLog2JDivj: sumMagLog2JDivj, determinant: determinant, a: a, bOrC: bOrC, d: d, x: x, y: y,l: l, magArrLength: magArrLength, LINE_COEFFS: LINE_COEFFS});
        ...
        for (let i = 0; i < lineFeatures.length; i++) {
            if (!isFinite(lineFeatures[i])) {
                this.log("NaN LF After", {magArr: magArr, lineFeatures: lineFeatures, sumMag: sumMag, sumMagDivj: sumMagDivj, sumMagLog2JDivj: sumMagLog2JDivj, determinant: determinant, a: a, bOrC: bOrC, d: d, x: x, y: y,l: l, magArrLength: magArrLength, LINE_COEFFS: LINE_COEFFS});
                this.logSend(MessageEnums.ERROR);
            }
        }
